<script lang="ts">
	...
	async function unstake() {
		...
		await backend.unstakeUSDC(input.replace(",", "."));
...
<div class="grid relative">
	...
	<div class="flex flex-col mt-2">
		<span class="text-3xl/7">Withdraw USD Coin</span>
	</div>
	...
	<div class="text-mid mt-1.5 flex gap-1">
		<img
			...
			alt="info"
		/>
		<span>{shortNot(eurRatio)} $ of USD Coin = 1 €</span>
	</div>
	...
	{#if orderState === 0}
		...
		<Button
			...
			>{reviewed ? "Withdraw USD Coin" : "Review order"}</Button
		>
	{:else}
		<div
		...
		>
			{#if orderState === 1}
				...
				<div class="text-lg">Withdrawing USDC</div>
			{:else}
