<script lang="ts">
	...
	async function stake() {
		...
		await backend.stakeDAI(input.replace(",", "."));
...
<div class="grid relative">
	...
	<div class="flex flex-col mt-2">
		<span class="text-3xl/7">Earn on Savings DAI</span>
		<span class="text-green-500">{shortNot($apyDAI)} % per year</span>
	</div>
	...
	<div class="text-mid mt-1.5 flex gap-1">
		<img
			...
			alt="info"
		/>
		<span>1 € = {shortNot(eurRatio)} $ of Savings DAI</span>
	</div>
	...
	{#if orderState === 0}
		...
		<Button
			...
			>{reviewed ? "Deposit Savings DAI" : "Review order"}</Button
		>
	{:else}
		<div
		...
		>
			{#if orderState === 1}
				...
				<div class="text-lg">Depositing DAI</div>
			{:else}
