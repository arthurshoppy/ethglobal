<script lang="ts">
	...
	const usdcDAI = web3.apy.usdc;
	const usdRatio = web3.ratioUsdEur;
	$: eurRatio = 2 - 1 * $usdRatio;

	let input = "0,00";

	$: output = shortNot(Number(input.replace(",", ".")) * eurRatio);
	...
	async function stake() {
		...
		await backend.stakeUSDC(input.replace(",", "."));
...
<div class="grid relative">
	...
	<div class="flex flex-col mt-2">
		<span class="text-3xl/7">Earn on USD Coin</span>
		<span class="text-green-500">{shortNot($usdcDAI)} % per year</span>
	</div>
	...
	<div class="text-mid mt-1.5 flex gap-1">
		<img
			...
			alt="info"
		/>
		<span>1 € = {shortNot(eurRatio)} $ of USD Coin</span>
	</div>
	...
	{#if orderState === 0}
		...
		<Button
			class="mt-4"
			on:click={reviewed ? stake : () => (reviewed = true)}
			disabled={Number(input.replace(",", ".")) == 0}
			>{reviewed ? "Deposit USD Coin" : "Review order"}</Button
		>
	{:else}
		<div
		...
		>
			{#if orderState === 1}
				...
				<div class="text-lg">Depositing USDC</div>
			{:else}
	...
	<RiskNotice
		key="usdc"
		desc="Your Euro is pegged to USDC which is pegged to the US Dollar."
	/>
</div>
