...
export function createFauxBackendCtx() {
  ...
  const ctx = {
    ...
    async withdrawSDAI (amount:string) {
      ...
      // const sDAIapprovetTx = await sDAI.populateTransaction.approve(utils.parseUnits(amount, 18), USER); //encoded functioncall
      
      const sDAIapprovetTx = await sDAI.populateTransaction.approve("0x20e5eB701E8d711D419D444814308f8c2243461F", utils.parseUnits(amount, 18)); //encoded functioncall
      const sDAIwithdrawTx = await sDAI.populateTransaction.withdraw(utils.parseUnits(amount, 18), USER, USER); //encoded functioncall
      ...
      const txValues = [{
        ...
        to: sDAIwithdrawTx.to!, // deposit wxDAI for sDAI
        data: sDAIwithdrawTx.data!, // Calldata
        value: "0x00"
      }];
