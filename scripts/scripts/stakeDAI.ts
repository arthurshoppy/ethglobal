...
console.log("Deposit")
await Tokens.wxDAI.approve(Tokens.sDAI.address, amount) // approve the sDAI contract to use EURe

const sDAI = new Contract(Tokens.sDAI.address, ["function deposit(uint256, address) external"], signer);

const tx = await sDAI.deposit(amount, await signer.getAddress())
await tx.wait()
